...
var (
	...
	serveCmd = &cobra.Command{
		Use:   "serve",
		Short: "Launches the svc-api-gateway service webserver",
...
func init() {
	...
	serveCmd.PersistentFlags().StringVar(&svcEchoAddress, "svc-echo-address", "", "Echo service address (host:port)")

	// Profile service address
	serveCmd.PersistentFlags().StringVar(&svcProfileAddress, "svc-profile-address", "", "Profile service address (host:port)")
...
func httpServer(httpServerAddr string, grpcServerAddr string, ctx context.Context, caCert string, gatewayCert string, gatewayKey string, jwtKey string) *http.Server {
	...
	n := negroni.New(
		...
		nlog.NewMiddlewareFromLogger(log.StandardLogger(), "svc-api-gateway"),
	)
