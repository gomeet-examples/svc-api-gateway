...
func (c *remoteCli) cmdListProfile(args []string) (string, error) {
	if len(args) < 6 {
		return "", errors.New("Bad arguments : list_profile <page_number [uint32]> <page_size [uint32]> <order [string]> <exclude_soft_deleted [bool]> <soft_deleted_only [bool]> <gender [UNKNOW|MALE|FEMALE]>")
	}
	...
	r, err := c.c.ListProfile(c.ctx, req)
	if err != nil {
		return "", fmt.Errorf("ListProfile service call fail - %v", err)
	}

	return fmt.Sprintf("ListProfile: %v", r), nil
}
