...
func (s *apiGatewayServer) AuthFuncOverride(ctx context.Context, fullMethodName string) (context.Context, error) {
	...
	if err != nil {
		if s.jwtSecret == "" ||
			fullMethodName == "/grpc.gomeetexamples.apigateway.ApiGateway/Version" ||
			fullMethodName == "/grpc.gomeetexamples.apigateway.ApiGateway/ServicesStatus" {
			return ctx, nil
		}
...
func (s *apiGatewayServer) AclFuncOverride(ctx context.Context, fullMethodName string, msg proto.Message) error {
	lFields := log.Fields{"fullMethodName": fullMethodName}
