...
func svcCurrentStatus(s *apiGatewayServer, ctx context.Context, svcCtx context.Context) *pb.ServiceStatus {
...
// func svc{{SubServiceNamePascalCase}}Status(s *apiGatewayServer, ctx context.Context, svcCtx context.Context) *pb.ServiceStatus {
...
func svcEchoStatus(s *apiGatewayServer, ctx context.Context, svcCtx context.Context) *pb.ServiceStatus {
	if err := s.initEchoClient(); err != nil {
		return serviceStatusErrorHandler(ctx, &pb.VersionResponse{"svc-echo", "unknow"}, err, "Init client error")
	}
	ver, err := s.svcEchoGrpcClient.Version(svcCtx, &svcEchoPb.EmptyMessage{})
	if err != nil {
		return serviceStatusErrorHandler(ctx, &pb.VersionResponse{"svc-echo", "unknow"}, err, "Version call error")
	}
	return serviceStatusSuccessHandler(&pb.VersionResponse{ver.GetName(), ver.GetVersion()})
}
func svcProfileStatus(s *apiGatewayServer, ctx context.Context, svcCtx context.Context) *pb.ServiceStatus {
	if err := s.initProfileClient(); err != nil {
		return serviceStatusErrorHandler(ctx, &pb.VersionResponse{"svc-profile", "unknow"}, err, "Init client error")
	}
	ver, err := s.svcProfileGrpcClient.Version(svcCtx, &svcProfilePb.EmptyMessage{})
	if err != nil {
		return serviceStatusErrorHandler(ctx, &pb.VersionResponse{"svc-profile", "unknow"}, err, "Version call error")
	}
	return serviceStatusSuccessHandler(&pb.VersionResponse{ver.GetName(), ver.GetVersion()})
}
...
func (s *apiGatewayServer) ServicesStatus(ctx context.Context, req *pb.EmptyMessage) (*pb.ServicesStatusList, error) {
	...
	fns := []func(*apiGatewayServer, context.Context, context.Context) *pb.ServiceStatus{
		svcCurrentStatus,
	...
	wg.Add(len(fns))
	for _, fn := range fns {
		go func(f func(*apiGatewayServer, context.Context, context.Context) *pb.ServiceStatus) {
			defer wg.Done()
