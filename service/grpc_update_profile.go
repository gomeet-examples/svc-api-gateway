...
func castUpdateProfileRequest(req *pb.ProfileInfo) (*svcProfilePb.ProfileInfo, error) {
	if req == nil {
	...
	return &svcProfilePb.ProfileInfo{
		Uuid:     req.GetUuid(),
		Gender:   gender,
		Email:    req.GetEmail(),
		Name:     req.GetName(),
...
func castUpdateProfileResponse(res *svcProfilePb.ProfileResponse) (*pb.ProfileResponse, error) {
	if res == nil {
...
func (s *apiGatewayServer) UpdateProfile(ctx context.Context, req *pb.ProfileInfo) (*pb.ProfileResponse, error) {
	...
	// cast request
	profileReq, err := castUpdateProfileRequest(req)
	if err != nil {
		log.Error(ctx, "castUpdateProfileRequest Failed", err, log.Fields{
			"req": req,
		})
	...
	//call profile.Update grpc service
	profileRes, err := s.svcProfileGrpcClient.Update(svcCtx, profileReq)
	if err != nil {
		log.Error(ctx, "Failed to svcProfileGrpcClient.Update call", err, log.Fields{
	...
	res, err := castUpdateProfileResponse(profileRes)
	if err != nil {
		log.Error(ctx, "castUpdateProfileResponse Failed", err, log.Fields{
			"req": req,
		})
